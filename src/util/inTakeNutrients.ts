...
export const inTakeNutrients = ({
  ...
  servingSize,
  nutriAmountServing,
}: calculateProps): MyNutrientType => {
  // dispatch 하기 전에 계산 해주기
  // 칼로리 계산
  const calorie = intake(nutrients.calorie, servingSize, nutriAmountServing);
  // 탄수화물 계산
  const carbohydrate = intake(
...
const intake = (
  ...
  if (!nutriAmountServing || !servingSize || nutriAmountServing === 0)
    return nutrient;
...
export const percent = (nutrients: MyNutrientType) => {
  ...
  // 칼로리 계산
  const calorie = (nutrients.calorie / 2000) * 100;
  // 탄수화물 계산
